...
/*
 * @author James Yang
 * This is the Boss state for our game, Boss Level and final level
 */
...
public class BossState extends GameState{
	...
	//private Slugger slugger; //the slugger enemy
	private HUD hud;
	...
	public void init() {
		...
		sfx.put("victory", new AudioPlayer("/Music/victory.mp3"));
		
		// backgrounds
		clouds = new Background("/Backgrounds/clouds.gif", 0.1);
		bg = new Background("/Backgrounds/bossbg.jpg", 0.2);
		...
		player.setPosition(40, 100);//sets the player position on the screen
		...
		bgMusic = new AudioPlayer("/Music/BossFight.mp3");//plays this song
		bgMusic.volume(-10);
...
//				if(e instanceof Slugger){ //if the enemy is a slugger
...
//		}
	...
	private void eventFinish(){
		...
		if(eventTime == 120) {
			tb.clear(); //clear existing boxes
			//create a dot rectangle
			tb.add(new Rectangle(
				GameLoop.WIDTH / 2, GameLoop.HEIGHT / 2, 0, 0));
		}
		else if(eventTime > 480 && eventTime < 540) {
			//the rectangle grows
			tb.get(0).x -= 6;
			tb.get(0).y -= 4;
			tb.get(0).width += 12;
		...
		else if(eventTime == 540){
			gsm.setState(GameStateManager.MENUSTATE);
			player.stop();
			sfx.get("victory").stop();
			eventTime = 0;
		}
	}
